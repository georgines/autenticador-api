 ...
 */
RFID::RFID(  byte chipSelectPin,   ///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
 ...
 */
void RFID::PCD_WriteRegister(  byte reg,   ///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
byte RFID::PCD_ReadRegister( byte reg  ///< The register to read from. One of the PCD_Register enums.
                ) {
  byte value;
  SPI.beginTransaction(SPISettings(SPI_CLOCK_DIV4, MSBFIRST, SPI_MODE0)); // Set the settings to work with SPI bus
  digitalWrite(_chipSelectPin, LOW);      // Select slave
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void RFID::PCD_ReadRegister( byte reg,   ///< The register to read from. One of the PCD_Register enums.
 ...
 */
void RFID::PCD_SetRegisterBitMask( byte reg, ///< The register to update. One of the PCD_Register enums.
 ...
 */
void RFID::PCD_ClearRegisterBitMask( byte reg, ///< The register to update. One of the PCD_Register enums.
 ...
 * Initializes the MFRC522 chip.
 */
bool RFID::PCD_Init( byte chipSelectPin,   ///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
            byte resetPowerDownPin  ///< Arduino pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low)
          ) {
  _chipSelectPin = chipSelectPin;
  _resetPowerDownPin = resetPowerDownPin; 
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
  */
RFID::StatusCode RFID::MIFARE_GetValue(byte blockAddr, long *value) {
  RFID::StatusCode status;
  byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_SetValue(byte blockAddr, long value) {
  byte buffer[18];
